...
function apiRequest(callback: (boolean, unknown) => void, path: string, method: string, headers, data: string | Record<any, any>) {
	$.ajax({
		"async": true,
		"type": method || "GET",
		"url": baseUrl + path,
		...
		onclick: function() {
			...
			for(var i = 0; i < spans.length; i++) {
				...
				if (!isNaN(original) && !isNaN(enchant.value) && !isNaN(level.value)) {
					...
					spans[i].innerHTML = sep + commafy(calculateStat(level.value, enchant.value, original));
				}
			}
		}
	});
