...
function p.CreateInfobox(frame)
    ...
    frame.args.EggName = frame.args.EggName and mw.text.decode(frame.args.EggName)
    if Namespace == '' and not Eggs[frame.args.EggName] then
        return '[[Category:Pages with incorrect name inputs]]' 
    end
    local RealPage = Namespace == '' and Eggs[frame.args.EggName] and true or false
    local Info = {
        title = (RealPage and frame.args.EggName) or frame.args.title1 or frame.args.title or frame:preprocess('{{PAGENAME}}'),
        image1 = frame.args.image1,
        caption1 = frame.args.caption1,
        Cost = (RealPage and Eggs[frame.args.EggName].cost) or (not RealPage and frame.args.cost or ''),
        Location = (RealPage and Eggs[frame.args.EggName].location) or (not RealPage and frame.args.location or '')
    }
    ...
    local Infobox = ('<div id="Eggs_%s"><infobox>'):format(mw.uri.encode(Info.title, 'WIKI'))
    ...
    return RealPage and ('[[Category:Eggs]]') or frame:preprocess('{{UserBlogWarn}}'), Infobox
end
