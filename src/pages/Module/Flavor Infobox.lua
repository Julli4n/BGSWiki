...
function p.CreateInfobox(frame)
    ...
    frame.args.EggName = frame.args.FlavorName and mw.text.decode(frame.args.FlavorName)
    if Namespace == '' and not Flavors[frame.args.FlavorName] then
        return '[[Category:Pages with incorrect name inputs]]' 
    end
    local RealPage = Namespace == '' and Flavors[frame.args.FlavorName] and true or false
    local Info = {
        title = (RealPage and frame.args.FlavorName) or frame.args.title1 or frame.args.title or frame:preprocess('{{PAGENAME}}'),
        image1 = frame.args.image1,
        caption1 = frame.args.caption1,
        Cost = (RealPage and Flavors[frame.args.FlavorName].cost) or (not RealPage and frame.args.cost or ''),
        Bubbles = (RealPage and Flavors[frame.args.FlavorName].bubbles) or (not RealPage and frame.args.bubble or '')
    }
    ...
    local Infobox = ('<div id="Flavors_%s"><infobox>'):format(mw.uri.encode(Info.title, 'WIKI'))
    ...
    return RealPage and ('[[Category:Flavors]]') or frame:preprocess('{{UserBlogWarn}}'), Infobox
end
